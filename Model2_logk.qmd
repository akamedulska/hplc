...
ggplot(df_logkCond)+ 
  ...
  #geom_line(aes(x = fi,  y = trCond2_Median, color = "blue"))+
  geom_ribbon(aes(x = fi, ymin = trCond1_Low, ymax = trCond1_High, color = analyte5), alpha = 0.25)+
  #geom_ribbon(aes(x = fi, ymin = trCond2_Low, ymax = trCond2_High, color = "blue"), alpha = 0.25)+
  facet_grid(pH~Mod,labeller = labeller(Mod=mod.labs,pH=pH.labs))+
  #scale_fill_discrete(name = "Analyte", labels = unique(data1$Name[which(data1$METID %in% index)]))+
