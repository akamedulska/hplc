...
fi <- rep(rep(seq(0,1,by=0.1),length(pH_value)),2*nAnalytes3)
pH <- rep(rep(pH_value,each=length(seq(0,1,by=0.1))*2),nAnalytes3)
fipH3 <-cbind(fi,pH)

mod3 <- rep(rep(c(2,1),each=length(seq(0,1,by=0.1))),2*nAnalytes3)
steps3=4*(2-mod3) + 10*(mod3-1)
hplcparam3=cbind(rep(gradient,by=length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3),
                 rep(2.1,by=length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3),
                 rep(0.532,by=length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3),
                 rep(0.04,by=length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3),
                 rep(0.05,by=length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3),
                 rep(0.8,by=length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3),
                 mod3,
                 rep(0,length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3),
                 rep(0,length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3),
                 rep(0,length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3),
                 (rep(temp,each=length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3)-25)/10)

nObs3 <- length(pH_value)*length(seq(0,1,by=0.1))*2*nAnalytes3
analyte3 <- rep(which(unique(data1$METID) %in% index),each=length(pH_value)*length(seq(0,1,by=0.1))*2)  # 1 - column (xbridge)
analyte4 <- rep(which(unique(data2$METID) %in% index),each=length(pH_value)*length(seq(0,1,by=0.1))*2)  # 2 - column (xterra)
analyte5 <- rep(c(1,2),each=length(pH_value)*length(seq(0,1,by=0.1))*2)  # 2 - column (xterra)
...
stan_rdump(c("nAnalytes1", 
             ...
             "pKasliterror3",
             "fipH3"),
           file="C:/Users/agnie/Desktop/HPLC_2022/1_25_09_combined/01_04_10/model_qq_1.data.R")

#model_qq <- cmdstan_model("C:/Users/agnie/Desktop/HPLC_2022/1_25_09_combined/01_04_10/hplc-gra-redsum_qsrr_04_10_qq_1.stan")
...
#fit_1_qq  <- model_qq$generate_quantities(fit_Combined,data = "C:/Users/agnie/Desktop/HPLC_2022/1_25_09_combined/01_04_10/model_qq_1.data.R",seed = 123)
#rm(fit_Combined,hplcparam1,hplcparam2,hplcparam3,hplcparam4,model_qq,data1,data2)
#fit_1_qq$save_object(file = "C:/Users/agnie/Desktop/HPLC_2022/1_25_09_combined/01_04_10/fit_1_qq.RDS")
fit_1_qq <- readRDS("C:/Users/agnie/Desktop/HPLC_2022/1_25_09_combined/01_04_10/fit_1_qq.RDS")
draws_1_qq_df <- fit_1_qq$draws(format = "df")
...
#| warning: false
ggplot(df_logkCond)+ 
  geom_line(aes(x = fi,  y = trCond1_Median, color =analyte5))+
  geom_ribbon(aes(x = fi, ymin = trCond1_Low, ymax = trCond1_High, color = analyte5), alpha = 0.25)+
  facet_grid(pH~Mod,labeller = labeller(Mod=mod.labs,pH=pH.labs))+
  scale_color_manual(labels =unique(data1$Name[which(data1$METID %in% index)]), values = c("red", "blue")) +
  labs(x ="fi", y = "log k", color = "Analyte")
```
...
#| warning: false
ggplot(df_logkCond)+ 
  geom_line(aes(x = fi,  y = trCond2_Median, color =analyte5))+
  geom_ribbon(aes(x = fi, ymin = trCond2_Low, ymax = trCond2_High, color = analyte5), alpha = 0.25)+
  facet_grid(pH~Mod,labeller = labeller(Mod=mod.labs,pH=pH.labs))+
  scale_color_manual(labels =unique(data1$Name[which(data1$METID %in% index)]), values = c("red", "blue")) +
  labs(x ="fi", y = "log k", color = "Analyte")
```
...
df_logkCond_Com <- cbind(rep(analyte5,2),df_logkCond_Com,fi,pH,rep(mod3,2),rep(c("Xbrigde Shield C18","XTerra C18"),each=88))
...
ggplot(df_logkCond_Com)+
    geom_line(aes(x = fi,  y = trCond_Median, color =analyte5))+
    geom_ribbon(aes(x = fi, ymin = trCond_Low, ymax = trCond_High, color = analyte5), alpha = 0.25)+
    facet_grid(pH~Mod+Column,labeller = labeller(Mod=mod.labs,pH=pH.labs))+
...
df_logkPred_Com <- cbind(rep(analyte5,2),df_logkPred_Com,fi,pH,rep(mod3,2),rep(c("Xbrigde Shield C18","XTerra C18"),each=88))
