...
ggplot(data = pp_B_pi, aes(x=variable, y=value)) + geom_boxplot(aes(fill=parameter_group),outlier.shape=NA)+ coord_flip()+
  theme(legend.position = "none")+ 
  scale_x_discrete(labels=functional_groups_names[,2])+
  labs(y="Marginal posterior distributions",x="")+facet_wrap(~parameter, labeller=lab_labeller,scales = "free_y", nrow = 1)
```
...
ggplot(data = pp_T_pi, aes(x=variable, y=value)) + geom_boxplot(aes(fill=parameter_group),outlier.shape=NA)+ coord_flip()+
  theme(legend.position = "none")+ 
  scale_x_discrete(labels=functional_groups_names[,2])+
  labs(y="Marginal posterior distributions",x="")+facet_wrap(~parameter, labeller=lab_labeller,scales = "free_y", nrow = 1)
```
